...
====You can configure from the `/api/v2/settings/logging/` endpoint how
the controller rsyslog process handles messages that have not yet been
sent in the event that your external logger goes offline:
...
====* *Logging Aggregator Type*: Click to select the aggregator service
