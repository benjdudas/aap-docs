...
[[ag_instance_groups_job_runtime_behavior]]
...
====[[ag_instance_groups_control_where_job_runs]]
...
`{ug_instance_groups}` in the Automation
...
image pull secrets.
====Once the container group is successfully created, the *Details* tab
of the newly created container group remains, which allows you to review
