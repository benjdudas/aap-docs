...
Clicking the *Job Templates* tab shows you the job templates associated
with this Credential and which jobs recently ran using this particular
credential.

image:credentials-home-with-jt-detail.png[image]

You can click the *Add* button to assign this *Demo Credential* to
additional job templates. Refer to the xref:ug_JobTemplates[] section for
further detail on creating a new job template.
...
Selecting this credential type allows you to create a credential that
gives the controller the ability to verify the integrity of the project
when syncing from source control.

image:credentials-create-gpg-credential.png[Credentials
- create GPG credential]

See {ug_content_signing} for detailed information on how to generate a
valid keypair, use the CLI tool to sign content, and how to add the
...
For more detail on this concept, refer to [.title-ref]#Understanding
...
====* *Privilege Escalation Username* field is only seen if an option
for privilege escalation is selected. Enter the username to use with
...
Private Keys, since the automation controller must first establish an
