...
job will prompt you for the value for that field upon launch. Most
prompted values will override any values set in the job template;
exceptions are noted below.
====[width="100%",cols="14%,53%,33%",options="header",]
|===
...
from the inventories available to the currently logged in user. |Yes.
Inventory prompts will show up as its own step in a subsequent prompt
window.

|*Project* |Choose the project to be used with this job template from
the projects available to the currently logged in user. |N/A
...
`{ug_job_branching}`. |Yes

|*Execution Environment* |Select the container image to be used to run
this job. A project must be selected before you can select an execution
environment. |Yes. Execution environment prompts will show up as its own
step in a subsequent prompt window.
...
|*Credentials* |Click the
image:search-button.png[search] button to
open a separate window. Choose the credential from the available options
to be used with this job template. Use the drop-down menu list to filter
by credential type if the list is extensive. Some credential types are
not listed because they do not apply to certain job templates. a|
* If selected, upon launching a job template that has a default
credential and supplying another credential will replace the default
credential if it is the same type. Example of such a message:
...
* Credential prompts will show up as its own step in a subsequent prompt
window.

|*Labels* a|
* Optionally supply labels that describe this job template, such as
"dev" or "test". Labels can be used to group and filter job templates
...
with that particular Job or Job Template, but it will remain associated
with any other jobs that reference it.
...
a|
* If selected, even if a default value is supplied, you will be prompted
...
must select *Prompt on Launch* for *Variables* on the job template, or a
enable a survey on the job template, then those answered survey
...
|*Verbosity* a|
Control the level of output Ansible produces as the playbook executes.
Choose the verbosity from Normal to various Verbose or Debug settings.
This only appears in the "details" report view. Verbose logging includes
the output of all commands. Debug logging is exceedingly verbose and
includes information on SSH operations that can be useful in certain
...
|*Timeout* a|
Allows you to specify the length of time (in seconds) that the job may
run before it is canceled. Some caveats for setting the timeout value:::
  * There is a global timeout defined in the settings which defaults to
  0, indicating no timeout.
  * A negative timeout (<0) on a job template is a true "no timeout" on
  the job.
  * A timeout of 0 on a job template defaults the job to the global
  timeout (which is no timeout by default).
...
|*Instance Groups* |Choose {ag_instance_groups} to associate with this
job template. If the list is extensive, use the
image:search-button.png[search] to narrow the
options. Note, job template instance groups contribute to the job
scheduling criteria, see {ag_instance_groups_job_runtime_behavior} and
{ag_instance_groups_control_where_job_runs} for rules. a|
* Yes. If selected, you are providing the jobs preferred instance groups
in order of preference. If the first group is out of capacity,
subsequent groups in the list will be considered until one with capacity
is available, at which point that will be selected to run the job.
* If you prompt for an instance group, what you enter replaces the
normal instance group hierarchy and overrides all of the organizations'
and inventories' instance groups.
* Instance Groups prompts will show up as its own step in a subsequent
prompt window.
...
must first save the template prior to launching, otherwise, the *Launch*
button remains grayed-out.

image:job-templates-job-template-details.png[image]
...
Launch* for *Variables* on the job template, or a configure and enable a
survey on the job template, then those answered survey questions become
`extra_vars`.
====[[ug_surveys]]
=== Surveys
...
====Below is an example job launch that prompts for Job Tags, and runs
the example survey created in xref:ug_surveys[].
...
* `awx_user_first_name`: The user's first name of the controller user
...
first run of it caches these facts and the next run will just pull them
...
meant for cloud bursting (i.e. new instances with a need for client to
...
Frequently this would be accessed via a firstboot type script, or from
cron.
...
====Most likely you will use callbacks with dynamic inventory in
Automation Controller, such as pulling cloud inventory from one of the
supported cloud providers. In these cases, along with setting _Update On
Launch_, be sure to configure an inventory cache timeout for the
inventory source, to avoid hammering of your Cloud's API endpoints.
Since the `request_tower_configuration.sh` script polls once per minute
for up to ten minutes, a suggested cache invalidation time for inventory
...
configuration upon coming online. To do so, running at first boot is a
better practice. First boot scripts are just simple init scripts that
