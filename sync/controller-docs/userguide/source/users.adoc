...
[arabic]
. Expand the *Access* menu from the left navigation bar, and click
*Users* to display a list of the current users.
...
[arabic, start=3]
. Click *Delete* in the confirmation warning message to permanently
delete the user.
...
aren't editable.
====This screen displays a list of the roles that are currently assigned
to the selected User and can be sorted and searched by *Name*, *Type*,
or *Role*.
